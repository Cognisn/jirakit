...
| `test_template_plan.py` | `reconcile_template`, `plan_template` and `missing_template_fields`: that a plan predicts what the apply then does, and that the field report reads only create metadata so it works without Jira administrator permission. |
