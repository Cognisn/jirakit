...
- Rollback retries deletions Jira refuses, for up to a new `retry_seconds` parameter (default 30 seconds, 0 to attempt each deletion once). Deleting a project does not release its schemes synchronously: against a live site, schemes were still refused as "assigned to one or more projects" more than ten minutes after the project had gone, while the API reported no project using them.
...
- Rollback also deletes the workflow scheme and workflow Jira creates alongside the project itself (`<KEY>: Software Simplified Workflow Scheme` and `Software Simplified Workflow for Project <KEY>`). Neither is tracked, and deleting the project does not remove them.
